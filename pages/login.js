...
const Login = () => {
    ...
    const handleOnLogin = (e)=>{
        ...
        let fetchedUser = user.filter(user=>user.email==userData.email.toLowerCase())[0];
        if(!fetchedUser){
            toast.error("User not found");
            return
        }
        if(fetchedUser.password!=userData.password){
            toast.error("Password incorrect")
            return;
        }
        if(typeof window!=='undefined'){
            toast.success("Login success");
            localStorage.setItem("tour-user", JSON.stringify(fetchedUser));
            router.push("/")
        }
                    ...
                    <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                            ...
                            <form onSubmit={handleOnLogin} className="space-y-4 md:space-y-6" action="#">
                                ...
                                <button type="submit" className={`w-full ${isEmailValid?'bg-blue-600 hover:bg-blue-700':'bg-blue-300'} text-white   focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800`}>Login</button>
