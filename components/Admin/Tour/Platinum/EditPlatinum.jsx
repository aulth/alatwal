...
const EditTour = ({ tourUrl }) => {
    const [tourData, setTourData] = useState({ title: "", overview: "", highlights: "", availability: "daily", status: 'active', description: "", category: "", location: '', duration: "", adultRatePrime: "", childRatePrime: "", infantRatePrime: "", adultRateNonPrime: "", childRateNonPrime: "", infantRateNonPrime: "", tourLanguage: "", importantInformation: "", bookingPolicy: "", tourVideo: "", tourAddress: "", googleMapLocation: "", featuredTour: true, paymentMethod: "merchant", platinum: true, id: '', success: false });
    ...
    const handleOnEdit = async (e) => {
        e.preventDefault();
        if (typeof window !== 'undefined') {
            if (!tourData.title || !image.length > 0 || !tourData.featuredTour || !tourData.availability || !tourData.bookingPolicy || !tourData.category || !tourData.adultRatePrime || !tourData.childRatePrime || !tourData.infantRatePrime || !tourData.adultRateNonPrime || !tourData.childRateNonPrime || !tourData.infantRateNonPrime || !tourData.description || !tourData.duration || !tourData.googleMapLocation || !tourData.highlights || !tourData.importantInformation) {
            ...
            const add = await fetch("/api/tour/update", {
                ...
                body: JSON.stringify({ title: tourData.title, overview: tourData.overview, highlights: tourData.highlights, availability: tourData.availability, status: tourData.status, description: tourData.description, category: tourData.category, duration: tourData.duration, adultRatePrime: tourData.adultRatePrime, childRatePrime: tourData.childRatePrime, infantRatePrime: tourData.infantRatePrime, adultRateNonPrime: tourData.adultRateNonPrime, childRateNonPrime: tourData.childRateNonPrime, infantRateNonPrime: tourData.infantRateNonPrime, tourLanguage: tourData.tourLanguage, importantInformation: tourData.importantInformation, bookingPolicy: tourData.bookingPolicy, tourVideo: tourData.tourVideo, tourAddress: tourData.tourAddress, googleMapLocation: tourData.googleMapLocation, featuredTour: tourData.featuredTour, paymentMethod: tourData.paymentMethod, authtoken: localStorage.getItem('alatwal-admin'), id: tourData.id, image: image, location: tourData.location, platinum:true })
            })
                ...
                <div className="w-full rounded border border-gray-300  my-6 box-border">
                    <h5 className="text-2xl font-semibold p-4 text-left border-b border-gray-300">Edit Tour</h5>
                    {
                        ...
                        <form onSubmit={handleOnEdit} className='w-full p-4 ' >
                            ...
                            <div className="w-full flex flex-col md:flex-row md:justify-between mb-6">
                                <label className='font-semibold flex items-center mr-2 md:mb-0 mb-1 w-52' htmlFor="">Adult Rate (Prime / Non Prime)  <sup className='text-red-600'>*</sup></label>
                                <div className="w-full flex items-center justify-center">
                                    <input type="number"  value={tourData.adultRatePrime} name="adultRatePrime" onChange={handleOnTextChange} placeholder="250" className='w-full focus:outline focus:outline-blue-400 p-1 rounded border mx-1' />
                                    <input type="number"  value={tourData.adultRateNonPrime} name="adultRateNonPrime" onChange={handleOnTextChange} placeholder="200" className='w-full focus:outline focus:outline-blue-400 p-1 rounded border mx-1' />
                                </div>
                            </div>
                            <div className="w-full flex flex-col md:flex-row md:justify-between mb-6">
                                <label className='font-semibold flex items-center mr-2 md:mb-0 mb-1 w-52' htmlFor="">Child Rate (Prime / Non Prime)  <sup className='text-red-600'>*</sup></label>
                                <div className="w-full flex items-center justify-center">
                                    <input type="number"  value={tourData.childRatePrime} name="childRatePrime" onChange={handleOnTextChange} placeholder="120" className='w-full focus:outline focus:outline-blue-400 p-1 rounded border mx-1' />
                                    <input type="number"  value={tourData.childRateNonPrime} name="childRateNonPrime" onChange={handleOnTextChange} placeholder="100" className='w-full focus:outline focus:outline-blue-400 p-1 rounded border mx-1' />
                                </div>
                            </div>
                            <div className="w-full flex flex-col md:flex-row md:justify-between mb-6">
                                <label className='font-semibold flex items-center mr-2 md:mb-0 mb-1 w-52' htmlFor="">Infant Rate  (Prime / Non Prime)  <sup className='text-red-600'>*</sup></label>
                                <div className="w-full flex items-center justify-center">
                                    <input type="number"  value={tourData.infantRatePrime} name="infantRatePrime" onChange={handleOnTextChange} placeholder="60" className='w-full focus:outline focus:outline-blue-400 p-1 rounded border mx-1' />
                                    <input type="number"  value={tourData.infantRateNonPrime} name="infantRateNonPrime" onChange={handleOnTextChange} placeholder="50" className='w-full focus:outline focus:outline-blue-400 p-1 rounded border mx-1' />
                                </div>
                            </div>
                            <div className="w-full flex flex-col md:flex-row md:justify-between mb-6">
