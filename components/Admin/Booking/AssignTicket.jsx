...
const AssignTicket = ({ bookingNumber }) => {
    ...
    return (
        <>
            <Head>
                <title>{bookingDetails ? bookingDetails.bookingFor == 'tour' ? 'Assign Ticket' : 'Upload Visa' : 'Assign'}</title>
            ...
            <div className="w-full p-4 overflow-y-auto">
                <div className="w-full flex justify-between">
                    <h6 className=" font-semibold ">{bookingDetails && `${bookingDetails.bookingFor=='tour'?'Assign Booking Ticket':bookingDetails.bookingFor=='visa'?'Upload Visa':'Assign'}`}</h6>
                    <button className="flex items-center text-[#1F41AF]"> <span className='text-gray-700 mr-1'>Bookings / <span className='mx-1 text-blue-500'>{bookingDetails && `${bookingDetails.bookingFor=='tour'?'Ticket':bookingDetails.bookingFor=='visa'?'Visa':''}`}</span> </span></button>
                </div>
